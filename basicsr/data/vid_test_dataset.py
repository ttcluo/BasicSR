...
class VidTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for clip_name in self.clip_list:
            ...
            self.data_info['key'].append(clip_name)
    ...
    def __getitem__(self, index):
        clip_name = self.data_info['key'][index]
...
if __name__ == '__main__':
    ...
    for idx, val_data in enumerate(dataloader):
        print('idx:{0}, video:{1}, lq_data:{2}, gt_data:{3}'.format(idx, val_data['key'], val_data['lq'].shape, val_data['gt'].shape))
