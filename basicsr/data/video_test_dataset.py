...
class VideoTestSVSRDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(VideoTestSVSRDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {'lq_path': [], 'gt_path': [], 'folder': [], 'idx': [], 'border': []}
        ...
        if 'meta_info_file' in opt:
            ...
            raise ValueError(f'No meta_info_file.')

        for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
            # get frame list for lq and gt
            # subfolder_name = subfolder_lq.split('/')[-2] + '/' + subfolder_lq.split('/')[-1]
            subfolder_name = osp.basename(subfolder_lq)
            img_paths_lq = sorted(glob.glob(osp.join(subfolder_lq, '*.png')))
            if len(img_paths_lq) == 0:
                img_paths_lq = sorted(glob.glob(osp.join(subfolder_lq, '*.jpg')))
            img_paths_gt = sorted(glob.glob(osp.join(subfolder_gt, '*.png')))
            if len(img_paths_gt) == 0:
                img_paths_gt = sorted(glob.glob(osp.join(subfolder_gt, '*.jpg')))
            ...
            if max_idx > lt:
                max_idx = lt
                img_paths_gt = img_paths_gt[:lt]
                img_paths_lq = img_paths_lq[:lt]
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            img_paths_lq = [self.imgs_lq[folder][i] for i in select_idx]
            imgs_lq = read_img_seq(img_paths_lq)
            img_gt = read_img_seq([self.imgs_gt[folder][idx]], gt=True)
            img_gt.squeeze_(0)
...
@DATASET_REGISTRY.register()
class VideoRecurrentTestSVSRDataset(VideoTestSVSRDataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames.
    ...
    def __init__(self, opt):
        super(VideoRecurrentTestSVSRDataset, self).__init__(opt)
        # Find unique folder strings
        self.folders = sorted(list(set(self.data_info['folder'])))
...
@DATASET_REGISTRY.register()
class VideoRecurrentTestSVSRPreDataset(data.Dataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames.
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        # Find unique folder strings
        self.folders = os.listdir(self.opt['dataroot_gt'])
        self.imgs_lq, self.imgs_gt = {}, {}
        for folder in self.folders:
            img_paths_lq = [self.opt['dataroot_lq'] + '/' + folder]
            img_paths_gt = [self.opt['dataroot_gt'] + '/' + folder]
            self.imgs_lq[folder] = read_img_seq(img_paths_lq)
            self.imgs_gt[folder] = read_img_seq(img_paths_gt)
