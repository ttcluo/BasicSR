...
class RealVSRRecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['val_partition'] == 'REALS50':
            val_partition = ['016', '018', '028', '029', '039', '044', '049', '088', '090', '108',
                        '135', '157', '159', '169', '170', '211', '212', '223', '245', '246',
                        '247', '250', '266', '276', '279', '289', '293', '295', '303', '317',
                        '323', '344', '346', '360', '362', '364', '374', '407', '429', '431',
                        '433', '449', '451', '458', '461', '471', '472', '484', '488', '492']
        elif opt['val_partition'] == 'official':
            val_partition = [f'{v:03d}' for v in range(240, 270)]
        else:
            raise ValueError(f'Wrong validation partition {opt["val_partition"]}.'
                             f"Supported ones are ['official', 'REDS4','RealS50'].")
        if opt['test_mode']:
            self.keys = [v for v in self.keys if v.split('/')[0] in val_partition]
        else:
            self.keys = [v for v in self.keys if v.split('/')[0] not in val_partition]
