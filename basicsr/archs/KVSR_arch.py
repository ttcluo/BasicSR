...
class KVSR(nn.Module):
    ...
    Args:
        num_feat (int): 特征通道数。默认: 64.
        num_block (int): 每个分支的残差块数量。默认: 15
        spynet_path (str): SPyNet 预训练权重的路径。默认: None.
    """
    ...
    def forward(self, x):
        ...
        out_l = [] # 存储后向特征
        feat_prop_bwd = x.new_zeros(b, self.num_feat, h, w) # 初始化后向传播特征
        ...
        for i in range(n - 1, -1, -1):
            ...
            # 特征提取
            feat_prop_bwd = torch.cat([x_i, feat_prop_bwd], dim=1) # 拼接当前帧和传播特征
            feat_prop_bwd = self.backward_trunk(feat_prop_bwd) # 通过残差块
            # 存储特征 (按正序插入)
            out_l.insert(0, feat_prop_bwd)

        # forward branch === 前向传播分支 (正序处理) ===
        feat_prop_fwd = torch.zeros_like(feat_prop_bwd)  # 重置前向传播特征
        out_frames = [] # 存储最终的输出帧
