...
class ICMEVSRx4_FG_SPM(nn.Module):
...
@ARCH_REGISTRY.register()
class ICMEVSRx4_FG_SPM_tmp(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper  IconVSRx2_FS_fg
    """
    ...
    def __init__(self,
        ...
        self.edvr = EDVRFeatureExtractor(temporal_padding * 2 + 1, num_feat, edvr_path)
        # alignment
        self.spynet = Raft_upNet000(spynet_path)  #  SpyNet  Raft_upNet000
    ...
    def forward(self, x):
        ...
        for i in range(0, n):
            ...
            feat_prop = torch.cat([x_i, out_l[i], feat_prop], dim=1)  #  spam_prop, 
            feat_prop = self.forward_trunk(feat_prop)
...
@ARCH_REGISTRY.register()
class ICMEVSRx4_warp_SPM(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper  IconVSRx2_FS_fg
    """
...
@ARCH_REGISTRY.register()
class ICMEVSRx4_FG_only(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper  IconVSRx2_FS_fg
    """
...
@ARCH_REGISTRY.register()
class ICMEVSRx4_warp_only(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper  IconVSRx2_FS_fg
    """
...
@ARCH_REGISTRY.register()
class ICMEVSRx4_FG_FB_SPM(nn.Module):
    """IconVSR, proposed also in the BasicVSR paper  IconVSRx2_FS_fg
    """
    ...
    def forward(self, x):
        ...
        for i in range(0, n):
            ...
            # print("[x_i new3]",x_i.shape) 
            x_up = F.interpolate(x_i, size=(self.scale*h_input, self.scale*w_input), mode='bilinear', align_corners=False)
...
class SecondOrderDeformableAlignment(ModulatedDeformConv2d):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conv_mask = nn.Sequential(
            nn.Conv2d(9 * self.deform_groups , self.out_channels, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1),
